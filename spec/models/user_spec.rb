...
RSpec.describe User, type: :model do
  describe '#create' do
    ...
    context 'ユーザー登録ができない時' do
      ...
      it 'first_name_katakanaが全角（漢字・ひらがな）の場合では登録できないこと' do
        @user.first_name_katakana = 'あいうえお'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name katakana is invalid')
      end

      it 'first_name_katakanaが英字のみでは登録できないこと' do
        @user.first_name_katakana = 'asdfgh'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name katakana is invalid')
      end
      ...
      it 'last_name_katakanaが全角（漢字・ひらがな）の場合では登録できないこと' do
        @user.last_name_katakana = 'あいうえお'
        @user.valid?
        expect(@user.errors.full_messages).to include('Last name katakana is invalid')
      end

      it 'last_name_katakanaが英字のみでは登録できないこと' do
        @user.last_name_katakana = 'asdfgh'
        @user.valid?
        expect(@user.errors.full_messages).to include('Last name katakana is invalid')
      end
